...
Upload videos to YouTube with pCloud shared links in the description.

### Upload new videos

```bash
uv run python batch_uploader.py --parent_dir /mnt/vault/WildPose_dataset
```

### Dry run (preview without uploading)

```bash
uv run python batch_uploader.py --parent_dir /mnt/vault/WildPose_dataset --dry_run
```

### Update descriptions of already-uploaded videos

Add pCloud shared links to existing YouTube videos:

```bash
uv run python batch_uploader.py --parent_dir /mnt/vault/WildPose_dataset --update_descriptions
```

Preview changes first:

```bash
uv run python batch_uploader.py --parent_dir /mnt/vault/WildPose_dataset --update_descriptions --dry_run
```

### Prerequisites

- YouTube OAuth2 token (see setup below)
- [rclone](https://rclone.org/) configured with `pCloud:` remote (for pCloud shared links)

...
ximea_cam_parameters = {
    ...
    "poll_time_frame": 1
    / 170,  # the ROS timer loop period (in seconds) for the ximea camera node. It should generally be set to a rate that is a factor higher than the camera capture rate.
    ...
    "calib_mode": False,
    "cam_context_path": "/home/naoya/WildPose_v1.1/wildpose/record/cam_context_{}.bin".format(
...
def generate_launch_description():
    ...
    gamepad_node = Node(
        ...
        parameters=[{"dev_name", "Wireless Gamepad F710"}],
        arguments=["--ros-args", "--log-level", "ERROR"],
    )
    ...
    rosbag = launch.actions.ExecuteProcess(
        cmd=[
            ...
            "/home/naoya/WildPose_v1.1/src/wildpose_bringup/config/reliability_override.yaml",
            # '--polling-interval', '0',
            "-o",
            os.path.join("./rosbags/", now.strftime("%Y%m%d_%H%M%S")),
        ],
        output="screen",
        on_exit=launch.actions.Shutdown(),
    )
